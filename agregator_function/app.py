        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for page_num in range(1, length_of_pdf + 1):
                page_items = get_items_from_record_table(
                    job_id=job_id, page_number=page_num, table=table
                )
            ...
            # Group pages by processor type
            processor_to_pages = {}
            for p in pages_metadata:
                proc = p.get("processor", "unknown").strip().lower()
                processor_to_pages.setdefault(proc, []).append(p)

            final_csv_results = []
            for proc, proc_pages in processor_to_pages.items():
                df = merge_csvs(proc_pages, bucket, temp_dir)
                if df is None or df.empty:
                    logger.info(f"No valid CSV data for processor '{proc}', skipping")
                    continue

                safe_proc = (
                    proc.replace(" ", "_").replace("+", "plus").replace("/", "-")
                )
                final_s3_key = (
                    f"final_outputs/{job_id}/{pdf_name}_{safe_proc}_final.csv"
                )
                save_csv_to_s3(df, bucket, final_s3_key)
