...
azure_table = os.getenv("AZURE_JOB_STATUS_TABLE")
gemini_table = os.getenv("GEMINI_JOB_STATUS_TABLE")


def get_items_from_record_table(job_id):

    table = dynamodb.Table(d_table)  # Name passed via env var

    status = ["FAILED", "INITIATED"]
    next = "split_pdf"
...
def update_azure_record_table(job_id: int, page_number: int, update_data: dict):
    """
    ...
    table = dynamodb.Table(azure_table)  # Name passed via env var
    ...
    # Remove None values and primary keys from update
    filtered_data = {
        k: v
...
def update_gemini_record_table(job_id: int, page_number: int, update_data: dict):
    """
    Update an existing record in DynamoDB without overwriting the whole item.
    Skips None values and primary keys to avoid deleting attributes unintentionally.
    """

    table = dynamodb.Table(gemini_table)  # Name passed via env var
